...
- For bugs, [file an
  issue](https://github.com/dieghernan/rasterpic/issues/new).
...
requests](https://github.com/dieghernan/rasterpic/issues) to make sure
the bug has not already been reported or fixed in the development
version. By default, the search is pre-populated with
...
**rasterpic** is maintained by volunteers, so a response may not be
immediate. Maintainers may address issues in batches, which can delay
responses. A good **reprex** is particularly important because it lets
us investigate a problem when we have capacity. If we cannot reproduce a
bug, we cannot fix it.
