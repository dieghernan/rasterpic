...
**terra** `SpatRaster` object (see `?terra::SpatRaster`).
...
This package allows you to create custom maps using a wide variety of spatial
objects:

- Spatial objects created with the **sf** package: `sf`, `sfc`, `sfg` or `bbox`.
- Spatial objects created with the **terra** package: `SpatRaster`, `SpatVector`
  and `SpatExtent`.
- A numeric coordinate vector of the form `c(xmin, ymin, xmax, ymax)`.

The main function, `rasterpic_img()`, is an S3 generic, so other packages can
...
We can also adjust other parameters and modify the alignment of the image with
